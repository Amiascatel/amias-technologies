...
export default function QuotationCreate({ nextNumber }: { nextNumber: string }) {
    const today = new Date().toISOString().slice(0, 10);
    const [items, setItems] = useState<LineItem[]>([blank()]);

    const { data, setData, post, processing } = useForm({
        quote_number: nextNumber, quote_date: today, validity_date: '', status: 'draft',
        client_name: '', client_company: '', client_address: '', client_phone: '', client_email: '',
        project_description: '', items: [blank()] as LineItem[],
        subtotal: 0, discount: 0, vat_rate: 16, vat_amount: 0, total: 0, currency: 'ZMW',
        notes: '', terms: 'This quotation is valid for 30 days from the date of issue.\nPrices are subject to change after the validity period.',
    });
    ...
    const addItem = () => recalc([...items, blank()], data.discount, data.vat_rate);
    const removeItem = (i: number) => recalc(items.filter((_, idx) => idx !== i), data.discount, data.vat_rate);

    const submit = (e: React.FormEvent) => { e.preventDefault(); post('/admin/quotations'); };

    return (
        <AppLayout breadcrumbs={[{ title: 'Quotations', href: '/admin/quotations' }, { title: 'New', href: '#' }]}>
            ...
            <div className="p-6 max-w-5xl space-y-6">
                ...
                <form onSubmit={submit} className="space-y-6">
                    <div className="bg-white rounded-xl border p-6 grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2 space-y-1"><Label>Project / Service Description</Label><Input value={data.project_description} onChange={e => setData('project_description', e.target.value)} /></div>
                    </div>
